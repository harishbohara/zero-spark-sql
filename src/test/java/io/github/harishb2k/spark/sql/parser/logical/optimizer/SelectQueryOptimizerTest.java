...
public class SelectQueryOptimizerTest {
     ...
     */
    @Test
    public void parseSelectQueryWithoutJoin_MergeJoinAndWhere() {
        String statement = """
              SELECT `id`, `name`, `created_at`
                 FROM `main_table`
                 WHERE `main_table`.`id` > 10;
     ...
     */
    @Test
    public void parseSelectQueryWithJoin_ProcessProjection() {
        String statement = """
                 SELECT `id`, `name`, `created_at`
                 FROM `main_table` INNER JOIN `other_table`
                    ON `main_table`.`column_name_main` = `other_table`.`column_name_other`
        ...
        optimizer.rules().add(new UnresolvedFromClauseWithSingleTable());
        optimizer.rules().add(new PredicatePushDownUnresolvedScan());
        optimizer.rules().add(new UnresolvedProjectionRule());
     ...
     */
    @Test
    public void parseSelectQueryWithoutJoin_ProcessProjection() {
        String statement = """
              SELECT `id`, `name`, `created_at`
                 FROM `main_table`
                 WHERE `main_table`.`id` > 10;
